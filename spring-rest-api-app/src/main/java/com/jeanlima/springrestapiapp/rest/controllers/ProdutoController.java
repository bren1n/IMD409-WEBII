...
public class ProdutoController {
    ...
    @PatchMapping("{id}")
    @ResponseStatus(NO_CONTENT)
    public void partialUpdate(@PathVariable Integer id, @RequestBody Produto produto) {
        repository.findById(id).map(p -> {
            BeanUtils.copyProperties(produto, p, patchHandler.getNullProperties(produto));
            repository.save(p);
            return p;
        }).orElseThrow(() ->
                new ResponseStatusException(HttpStatus.NOT_FOUND,
